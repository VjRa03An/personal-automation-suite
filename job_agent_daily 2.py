...
Sources: WWR, Remotive, Jobicy, RemoteOK, Dice, LinkedIn RSS
"""
...
# ─────────────────────────────────────────────────────────────
def scrape_wwr():
    # Only categories relevant to TPM/PMO/Ops executive roles
    feeds = [
...
def scrape_remotive():
    # Only management category — no product/devops noise
    jobs = []
    for cat in ['management']:
        log(f"  Remotive → {cat}")
        try:
            r = requests.get(f"https://remotive.com/api/remote-jobs?category={cat}&limit=100",
...
def scrape_jobicy():
    queries = [
        ...
        ("vp+technical+program",               "VP TPM"),
        ("head+of+engineering+operations",     "Head Eng Ops"),
    ]
    jobs = []
...
def scrape_remoteok():
    """RemoteOK API — good signal for senior tech-adjacent roles."""
    log("  RemoteOK → senior program management")
    jobs = []
    try:
...
def scrape_dice():
    """Dice RSS — good for tech director/TPM roles in the US."""
    queries = [
        ("Director+Technical+Program+Manager", "Director TPM"),
        ("Director+PMO",                        "Director PMO"),
        ("Chief+of+Staff+Engineering",          "CoS Eng"),
        ("Director+Product+Operations",         "Director Prod Ops"),
    ]
    jobs = []
    for kw, label in queries:
        log(f"  Dice → {label}")
        try:
            url = f"https://www.dice.com/jobs/q-{kw}-jobs"
            # Dice exposes an API endpoint
            api = f"https://job-search-api.svc.dhigroupinc.com/v1/dice/jobs/search?q={kw}&countryCode2=US&pageSize=20&shortDescription=true&sortBy=postedDate&sortDirection=DESC"
            r = requests.get(api, headers={**HEADERS, "Accept": "application/json"}, timeout=10)
            if r.status_code == 200:
                for j in r.json().get('data',[]):
                    jobs.append({
                        'title':   j.get('title',''),
                        'company': j.get('advertiserName','Unknown'),
                        'source':  'Dice',
                        'location': j.get('location','USA'),
                        'url':     f"https://www.dice.com/job-detail/{j.get('id','')}",
                        'posted':  j.get('postedDate','Recent')[:10],
                        'desc':    strip_html(j.get('shortDescription','') or j.get('description',''))[:500],
                    })
        except Exception as ex: log(f"     ✗ {ex}")
        time.sleep(0.4)
    return jobs
...
def scrape_linkedin_rss():
    """LinkedIn public RSS feeds for TPM/PMO roles."""
    queries = [
        ("director+technical+program+manager", "Director TPM"),
        ("director+pmo",                       "Director PMO"),
        ("chief+of+staff+engineering",         "CoS Eng"),
        ("director+product+operations",        "Director Prod Ops"),
        ("vp+technical+program+management",    "VP TPM"),
    ]
    jobs = []
    for kw, label in queries:
        log(f"  LinkedIn → {label}")
        try:
            rss_url = f"https://www.linkedin.com/jobs/search.rss?keywords={kw}&location=United+States&f_TPR=r86400&sortBy=DD"
            feed = feedparser.parse(rss_url, request_headers=HEADERS)
            for e in feed.entries:
                title = getattr(e,'title','').strip()
                if not title: continue
                desc = strip_html(getattr(e,'summary','') or '')
                jobs.append({'title':title,'company':extract_company(e),'source':'LinkedIn',
                    'location':'USA','url':getattr(e,'link','#'),
                    'posted':fmt_date(e),'desc':desc[:500]})
        except Exception as ex: log(f"     ✗ {ex}")
        time.sleep(0.5)
    return jobs
...
# ─────────────────────────────────────────────────────────────
# RELEVANCE FILTER
# Two-pass: hard title exclude → seniority check → function check
# ─────────────────────────────────────────────────────────────

# If the title contains ANY of these → reject immediately
EXCLUDE_TITLE_KW = [
    ...
    'marketing', 'designer', 'ux ', 'ui ', 'researcher',
...
# Title must contain one of these function keywords OR description must be rich with them
TITLE_FUNCTION_KW = [
    ...
    'product operations',
    'portfolio management', 'portfolio program',
    'chief of staff',
    'transformation',
]
...
def is_relevant(job):
    ...
    # Hard exclude on title first
    if any(k in title for k in EXCLUDE_TITLE_KW):
        return False

    # Must have seniority in title
    if not any(k in title for k in SENIORITY_KW):
        return False

    # Function check: TPM/PMO keyword in title (preferred)
    if any(k in title for k in TITLE_FUNCTION_KW):
        return True

    # Fallback: strong TPM/PMO signal in description
...
# ─────────────────────────────────────────────────────────────
# DEBUG: print filter decisions (use --debug flag)
# ─────────────────────────────────────────────────────────────
...
<div style="max-width:620px;margin:0 auto;padding:24px 16px;">
  ...
  <div style="text-align:center;font-size:11px;color:#9ca3af;margin-top:24px;padding-top:16px;border-top:1px solid #e5e7eb;">
    Job Curation Agent · Sources: WWR, Remotive, Jobicy, RemoteOK, Dice, LinkedIn · {run_date}
  </div>
</div>
</body></html>"""
...
def run_agent(debug=False):
    ...
    raw = (scrape_wwr() + scrape_remotive() + scrape_jobicy() +
           scrape_remoteok() + scrape_dice() + scrape_linkedin_rss())
    log(f"\nRaw total: {len(raw)}")
